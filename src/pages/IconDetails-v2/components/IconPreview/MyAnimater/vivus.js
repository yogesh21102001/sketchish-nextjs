...
VivusInstant.prototype.setElement = function (element, options) {
  ...
  this.styleTag.setAttribute('data-made-with', 'openStroke-instant')
  this.el.appendChild(this.styleTag);
  this.el.setAttribute('class', `${options?.type}`);

  // Get all child elements within the parent
  const childElements = this.el.children;

  // Set the attribute for each child element
  for (let i = 0; i < childElements.length; i++) {
    if (childElements[i].tagName !== "STYLE") {
      childElements[i].setAttribute("data-tag", "path");
    }
  }

  this.addOtherAnimations(options)
  this.preMapping();
};
...
VivusInstant.prototype.preMapping = function () {
  var i, path, pathObj,
    paths = this.el.querySelectorAll("[data-tag = 'path']") || document.createElement('path');
  console.log(paths);
  this.map = [];
...
VivusInstant.prototype.setOptions = function (options) {
  var allowedTypes = ['delayed', 'async', 'oneByOne', 'scenario', 'scenario-sync', 'bumping', 'beatfade', 'shake', 'beat', 'flip', 'rotate'];
...
VivusInstant.prototype.mapping = function () {
  ...
  for (i = 0; i < this.map.length; i++) {
    ...
    switch (this.type) {
      ...
      case 'scenario-sync':
        pAttrs = this.parseAttr(pathObj.el);
...
VivusInstant.prototype.addOtherAnimations = function (options) {
  const beat = `
     @keyframes beat{0%{transform: scale(1);}50%{transform:scale(1.3);}100%{transform: scale(1);}}.${options?.type}{width:48px;height:48px;animation:beat 1s infinite;}`;
  const beatFade = `
     @keyframes beat-fade{0%,100%{opacity:0.6;transform: scale(1);}50%{opacity: 1;transform: scale(1.3);}}.${options?.type}{animation:beat-fade 2s infinite;}`;
  const shake = `
     @keyframes shake {0% {transform: translateX(0);}20% {transform: translateX(-5px);}40% {transform:translateX(5px);}60%{transform: translateX(-5px);}80%{transform: translateX(5px);}100%{transform: translateX(0);}}.${options?.type}{animation:shake .5s infinite;}`;
  const bumping = `
     @keyframes bounce{0%,20%,50%,80%,100%{transform: translateY(0);}40%{transform:translateY(-20px);}60%{transform: translateY(-10px);}}.${options?.type}{animation:bounce 2s infinite;}`;
  const flip = `
     @keyframes flip{0%{transform: rotateY(0deg);}100%{transform:rotateY(180deg);}}.${options?.type}{animation:flip 1s infinite;}`;
  const rotate = `
     @keyframes rotate{0%{transform: rotate(0deg);}100%{transform:rotate(360deg);}}.${options?.type}{animation:rotate 5s linear infinite;}`;

  switch (options?.type) {
    case 'beat': 
      this.styleTag.innerHTML = beat;
      break;

    case 'beatfade': 
      this.styleTag.innerHTML = beatFade;
      break;

    case 'shake': 
      this.styleTag.innerHTML = shake;
      break;

    case 'bumping': 
      this.styleTag.innerHTML = bumping;
      break;

    case 'flip': 
      this.styleTag.innerHTML = flip;
      break;

    case 'rotate': 
      this.styleTag.innerHTML = rotate;
      break;
  }
